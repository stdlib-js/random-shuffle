...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0qf2V,FD', options ) );
	}
	if ( hasOwnProp( options, 'copy' ) ) {
		opts.copy = options.copy;
		if ( !isString( opts.copy ) ) {
			return new TypeError( format( '0qf2W,Gh', 'copy', opts.copy ) );
		}
		if ( indexOf( COPY_OPTIONS, opts.copy ) === -1 ) {
			return new TypeError( format( '0qf2X,3g,4S,GD,Gg,Jm', 'copy', COPY_OPTIONS.join( '", "' ), opts.copy ) );
		}
	}
	return null;
}
