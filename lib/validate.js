...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0qf2V', options ) );
	}
	if ( hasOwnProp( options, 'copy' ) ) {
		opts.copy = options.copy;
		if ( !isString( opts.copy ) ) {
			return new TypeError( format( '0qf2W', 'copy', opts.copy ) );
		}
		if ( indexOf( COPY_OPTIONS, opts.copy ) === -1 ) {
			return new TypeError( format( '0qf4S', 'copy', COPY_OPTIONS.join( '", "' ), opts.copy ) );
		}
	}
	return null;
}
